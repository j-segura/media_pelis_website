<x-app-layout>
    <x-nav_bar_admin>
        <div class="admin_tables">
            ...
            <form action="{{ route('productor_update', $productor) }}" method="POST">
                @csrf
                @method('put')
    
                <div class="mb-6">
                    <label for="name" class="block mb-2 text-sm font-medium text-black dark:text-black">Actual nombre del Productor</label>
                    <input id="name" type="text" name="name" value="{{ old('name', $productor->name) }}"
