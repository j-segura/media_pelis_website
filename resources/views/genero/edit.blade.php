    ...
    <x-nav_bar_admin>
        <div class="admin_tables">
            <div class="header_crear">
                <a href="{{ route('generos_index') }}" class="volver_btn">
            ...
            <form action="{{ route('genero_update', $genero) }}" method="POST">
                @csrf
                @method('put')
    
                <div class="mb-6">
                    <label for="name" class="block mb-2 text-sm font-medium text-black dark:text-black">Actual nombre del Genero</label>
                    <input id="name" type="text" name="name" value="{{ old('name', $genero->name) }}"
